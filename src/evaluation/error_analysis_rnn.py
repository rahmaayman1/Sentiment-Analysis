...
def run_error_analysis():
    ...
    model.load_state_dict(torch.load("models/rnn/best_rnn.pt", map_location=DEVICE))
    model.eval()
    ...
    with open("reports/rnn/error_analysis.txt", "w", encoding="utf-8") as f:
        f.write("=" * 60 + "\n")
        f.write("ERROR ANALYSIS — RNN\n")
        f.write("=" * 60 + "\n\n")
        ...
        f.write("NOTE\n")
        f.write("=" * 60 + "\n")
        f.write("RNN accuracy ~50% indicates vanishing gradient problem.\n")
        f.write("Long sequences (MAX_LEN=200) cause gradients to vanish\n")
        f.write("before reaching early tokens. This motivated using LSTM.\n")

    print(" Saved to reports/rnn/error_analysis.txt")
