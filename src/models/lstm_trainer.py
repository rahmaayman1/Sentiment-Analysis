...
def main():
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(model.state_dict(), "models/lstm/best_lstm.pt")
            print(f"   Best model saved (val_acc: {val_acc:.4f})")

    # Test
    model.load_state_dict(torch.load("models/lstm/best_lstm.pt"))
    test_loss, test_acc = evaluate(model, test_loader, criterion)
    print(f"\n=== Test Accuracy: {test_acc:.4f} ===")

    # Save Report
    os.makedirs("reports/lstm", exist_ok=True)
    with open("reports/lstm/lstm_results.txt", "w", encoding="utf-8") as f:
        f.write("=" * 60 + "\n")
        f.write("LSTM TRAINING RESULTS\n")
        f.write("=" * 60 + "\n\n")
    ...
    print(" Saved to reports/lstm/lstm_results.txt")
